...
use peterparker_core::models::{Device, Port};
use crate::DeviceStore;

#[tauri::command]
pub async fn get_devices(device_store: State<'_, DeviceStore>) -> Result<Vec<Device>, String> {
    let store = device_store.read().await;
    Ok(store.clone())
}

#[tauri::command]
pub async fn get_device(ip: String, device_store: State<'_, DeviceStore>) -> Result<Option<Device>, String> {
    let store = device_store.read().await;
    Ok(store.iter().find(|d| d.ip == ip).cloned())
}

#[tauri::command]
pub async fn delete_device(id: String, device_store: State<'_, DeviceStore>) -> Result<(), String> {
...
#[tauri::command]
pub async fn rescan_device(ip: String, device_store: State<'_, DeviceStore>) -> Result<Device, String> {
    let mut device = Device::new(ip.clone());
    let ports = quick_scan_ports(&ip, &peterparker_core::models::port::TOP_PORTS[..20]).await;
    device.ports = ports;
    device.classify();
